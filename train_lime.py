...
def prepare_category_lifetime_map(config, corpus):
    mapping = torch.full(
        (config.category_num + 1,),
        float(config.default_lifetime),
        dtype=torch.long,
        device=config.device,
    )
    for category_name, category_index in corpus.category_dict.items():
        mapping[category_index] = config.topic_wise_lifetime.get(category_name, config.default_lifetime)
    config.category_lifetime_map = mapping
...
def main():
    ...
    prepare_category_lifetime_map(config, corpus)
    ...
    if config.mode == "train":
        ...
        raise ValueError("train_lime.py supports train and test modes only")
    ...
    write_results_summary(
        summary_path,
        {
            ...
            "ndcg5": ndcg5,
            "ndcg10": ndcg10,
            "lifetime_type": config.lifetime_type,
        },
    )
    ...
    print(f"lifetime_type={config.lifetime_type}")
    if auc is not None:
        print(f"auc={auc:.4f}")
        print(f"mrr={mrr:.4f}")
